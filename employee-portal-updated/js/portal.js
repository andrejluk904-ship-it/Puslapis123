...
function entriesToSelectedDays(entries = []) {
  selectedScheduleDays = new Set(
    entries
      .filter(e => Number(e.hours) > 0 && e.date)
      .map(e => normalizeDateValue(e.date))
  );
}

function buildEntriesFromSelectedDays() {
  const dates = Array.from(selectedScheduleDays).sort();
  let total = 0;
...
function setMonthFromSchedule(schedule) {
  ...
  const firstEntryDate = schedule?.entries?.find(e => e.date)?.date;
  if (firstEntryDate) {
    monthInput.value = normalizeDateValue(firstEntryDate).slice(0, 7);
  } else if (!monthInput.value) {
    const now = new Date();
    monthInput.value = `2026-${String(now.getMonth() + 1).padStart(2, '0')}`;
  }
...
async function loadScheduleForEmployee(employeeId) {
  ...
  try {
    const schedule = await api(`/api/schedules/employee/${employeeId}`);
    ...
    if (schedule) {
      ...
      renderMonthCalendar();
      $("#schedulePreview").innerHTML = '<div class="muted">Nėra grafiko šiam darbuotojui.</div>';
      if (deleteBtn) deleteBtn.style.display = 'none';
    }
  } catch (err) {
  ...
  if (csvFile) {
    csvFile.addEventListener("change", async (e) => {
      ...
      try {
        ...
        await api('/api/schedules', {
          method: 'POST',
          body: JSON.stringify({ employeeId, period, entries, updateType: 'csv' })
        });

        alert(`Grafikas importuotas (${entries.length} įrašų)`);
        await loadScheduleForEmployee(employeeId);
        csvFile.value = '';
      } catch (err) {
        alert('Klaida: ' + err.message);
    ...
    imgFile.addEventListener("change", async (e) => {
      ...
      try {
        await api('/api/schedules', {
          ...
          body: JSON.stringify({
            employeeId,
            period: currentSchedule?.period || 'Nuotrauka',
...
async function loadPayrollForEmployee(employeeId) {
  ...
  try {
    const payroll = await api(`/api/payroll/employee/${employeeId}`);
    if (payroll) {
...
async function savePayroll(mode) {
  ...
  if (isAuto) {
    ...
    const hoursMatch = infoText.match(/([\d.]+) val\. iš viso/);
    const otMatch    = infoText.match(/\(([\d.]+) viršval\.\)/);
    const totalH  = hoursMatch ? parseFloat(hoursMatch[1]) : 0;
    const otH     = otMatch    ? parseFloat(otMatch[1])    : 0;
    const regH    = Math.round((totalH - otH) * 100) / 100;
    data = {
      employeeId, period,
      hours: regH,
...
async function autoCalcFromSchedule() {
  ...
  try {
    const sched = await api(`/api/schedules/employee/${employeeId}`);
    ...
    if (infoEl) {
      ...
      infoEl.textContent = `✅ Periodas: ${period} • ${workDays} darbo dienos • ${totalHours} val. iš viso${overtimeHours > 0 ? ` • Viršvalandžiai: ${overtimeHours} val.` : ' • Viršvalandžių nėra'}`;
    }

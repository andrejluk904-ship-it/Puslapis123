...
async function loadSchedule() {
  try {
    schedule = await api(`/api/schedules/employee/${currentUser.employeeId}`);
    renderSchedule();
  } catch (err) {
...
async function loadPayroll() {
  try {
    payroll = await api(`/api/payroll/employee/${currentUser.employeeId}`);
    renderPayroll();
  } catch (err) {
...
function renderSchedule() {
  ...
  // Rodome 2026 metus. Jeigu grafikas turi įrašų, imame pirmą įrašo mėnesį.
  const months = [...new Set(validEntries.map(e => e.date.slice(0, 7)))].sort();
...
function renderPayroll() {
  ...
  if (payPeriod) payPeriod.textContent = payroll.period;
...
function renderAutoPayrollFromSchedule() {
  ...
  // Build calendar-style week grid
  const scheduleCard = $("#payroll .card");
  const autoCalcEl = $("#pay_auto_calc");

  if (!autoCalcEl && scheduleCard) {
    const div = document.createElement("div");
    div.id = "pay_auto_calc";
    div.style.cssText = "margin-top:16px; padding:12px; background:#f0fdf4; border:1px solid #bbf7d0; border-radius:10px;";
    div.innerHTML = `
      <div style="font-size:12px; color:#166534; font-weight:600; margin-bottom:10px;">⚡ Automatinis skaičiavimas iš grafiko (periodas: ${escapeHtml(schedule.period || '—')})</div>
      <div style="display:grid; grid-template-columns:repeat(auto-fill,minmax(80px,1fr)); gap:6px; margin-bottom:12px;">
        ${workDays.map(e => `
          <div style="text-align:center; padding:6px 4px; background:white; border-radius:6px; border:1px solid #d1fae5; font-size:11px;">
            <div style="color:#6b7280; margin-bottom:2px;">${escapeHtml(e.day || '')}</div>
            <div style="font-weight:600; color:#111;">${escapeHtml(e.date ? e.date.slice(5) : '')}</div>
            <div style="color:#0f766e; font-size:12px; font-weight:700;">${e.hours} val.</div>
          </div>
        `).join('')}
      </div>
      <div style="display:flex; gap:16px; flex-wrap:wrap; font-size:13px; padding-top:10px; border-top:1px solid #bbf7d0;">
        <div><span style="color:#6b7280;">Iš viso:</span> <strong>${Math.round(totalHours * 100) / 100} val.</strong></div>
        <div><span style="color:#6b7280;">Darbo dienų:</span> <strong>${workDays.length}</strong></div>
        ${overtimeHours > 0 ? `<div><span style="color:#6b7280;">Viršvalandžiai:</span> <strong>${Math.round(overtimeHours * 100) / 100} val.</strong></div>` : ''}
      </div>
      <div style="margin-top:8px; font-size:11px; color:#6b7280;">💡 Tikslią algą apskaičiuos administracija pagal valandinį įkainį.</div>
    `;

    const resultEl = scheduleCard?.querySelector('.result');
    if (resultEl) {
      scheduleCard.insertBefore(div, resultEl);
    } else {
      scheduleCard?.appendChild(div);
    }
  }
  ...
  if (payHours) payHours.textContent = `${Math.round(totalHours * 100) / 100} val. (iš grafiko)`;
  if (payBruto) payBruto.textContent = "—";
  if (payTotal) payTotal.textContent = "—";
